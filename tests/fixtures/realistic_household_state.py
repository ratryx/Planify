...
def inject_realistic_household(plan: WorkbookPlan, year: int):
    ...
    add_row(tbl_contas, Nome="Poupança", Tipo="Poupança", Instituição="Banco B", **{"Saldo inicial": 15000, "Incluir no saldo disponível?": "Não", "Ativa?": "Sim"})
    ...
    # Refund
    add_row(tbl_lancamentos, Data=date(year, 1, 22), Descrição="Estorno TV", Tipo="Estorno / Reembolso", Categoria="Lazer", Cartão="Cartão Principal", **{"Competência da fatura": date(year, 1, 1), "Valor": 3000})
    ...
    add_row(tbl_dividas, Dívida="Financiamento Carro", Categoria="Financiamento de veículo", Credor="Banco A", **{"Saldo devedor atual": 25000, "Parcela mensal atual": 800, "Data final": date(year+2, 5, 1)})
    add_row(tbl_dividas, Dívida="Empréstimo Família", Categoria="Dívida com pessoa", Credor="Tio", **{"Saldo devedor atual": 5000, "Parcela mensal atual": 200})
